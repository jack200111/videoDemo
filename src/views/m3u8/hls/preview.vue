  ...
  methods: {
    ...
    changeLayout({ nums }) {
      ...
      if (nums < this.checkVideoList.length) {
        ...
        const destroyVideoIdList = this.checkVideoList.slice(nums, this.checkVideoList.length).map((item) => 'videoPlayer' + item.userId + item.sessionID);
        // 右侧渲染的视频列表 改为可选
    ...
    changeVideoIndex(video, event) {
      ...
      if (event) {
        ...
        this.checkVideoList = [...this.checkVideoList, video]
        // 添加视频实例
        ...
        // 取消勾选
        // 中间播放视频的数组 减去一条视频
        this.checkVideoList = this.checkVideoList.filter((item) => 'videoPlayer' + item.userId + item.sessionID !== videoid)
    ...
    getStartPush() {
      videoApi.getLiveList().then((response) => {
        if (response.data.length > 0) {
          ...
          // 模拟初始化添加
          const video1 = this.deviceInfoList[0]
          const video2 = this.deviceInfoList[1]
          // 添加视频实例的id
          const videoid = 'videoPlayer' + video1.userId + video1.sessionID
          const videoid2 = 'videoPlayer' + video2.userId + video2.sessionID
          // 初始化视频
          this.$refs.videoBox.initPlayer(videoid, video1.hlsUrl)
          this.$refs.videoBox.initPlayer(videoid2, video2.hlsUrl)
          // 修改右侧选中复选框
          video1.isChecked = true
          video2.isChecked = true
          // 添加中间播放视频的数组
          this.checkVideoList = [...this.checkVideoList, video1, video2]
          // 添加选中列表
          this.checkDeviceInfoList.push(video1, video2)
...
.dashboard-editor-container {
  ...
  .card_box {
    ...
    height: calc(100vh - (85px + 16px + 16px ));
  }
  .card_box:hover {
