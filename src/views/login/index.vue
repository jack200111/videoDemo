          ...
          this.$store.dispatch('user/login', this.loginForm).then(async(res) => {
            ...
            if (res.data) {
              ...
              if (re.data) {
                ...
                // 存系统
                // 加密
                localStorage.setItem('menuList', crypto.encrypt(re.data))
              }
            }
            // 请求动态路由菜单
            this.$store.commit('user/SET_NAME', res.data.account)
            menuApi.getMenuTreeAll().then((response) => {
              ...
              // this.getUserList();
              // 提交到仓库
              console.log(this.$store.getters.userMenu, 'store.getters.userMenu');
